...
public class KamiActiveModulesUI extends AbstractComponentUI<ActiveModules> {
    ...
    public void renderComponent(ActiveModules component, FontRenderer f) {
        ...
        mods.stream().forEach(module -> {
            ...
            int blue = rgb & 0xFF;
            if(((ModuleList)ModuleManager.getModuleByName("ModuleList")).colorMode.getValue().equals(ColorMode.RAINBOW)) {
            renderer.drawStringWithShadow(xFunc.apply(textwidth), y[0], red,green,blue, text);
            hue[0] +=.02f;
            y[0] += textheight;
            }
            int modRedS = (((ModuleList)ModuleManager.getModuleByName("ModuleList")).listRed.getValue());
            int modBlueS = (((ModuleList)ModuleManager.getModuleByName("ModuleList")).listBlue.getValue());
            int modGreenS = (((ModuleList)ModuleManager.getModuleByName("ModuleList")).listGreen.getValue());
            if(((ModuleList)ModuleManager.getModuleByName("ModuleList")).colorMode.getValue().equals(ColorMode.STATIC)) {
            renderer.drawStringWithShadow(xFunc.apply(textwidth), y[0], modRedS,modGreenS,modBlueS, text);
            hue[0] +=.02f;
            y[0] += textheight;
            }
        });
